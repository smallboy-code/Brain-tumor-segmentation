...
# train
train_set = {
        'root': '/mnt/data/data/BraTs2021/RSNA_ASNR_MICCAI_BraTS2021_TrainingData_16July2021',
        'flist': 'brats2021/train.txt',
        'has_label': True
        }

# test/validation data
valid_set = {
        'root': '/mnt/data/data/BraTs2021/RSNA_ASNR_MICCAI_BraTS2021_ValidationData',
        'flist': 'brats2021/valid.txt',
        'has_label': False
        }

test_set = {
        'root': '/media/dmia/data11/hh/MICCAI_BraTS2019_ValidationData',
        'flist': 'brats2020_new/test.txt',
        'has_label': False
        }
...
def process_f32b0(path, has_label=True):
    ...
    output ='brats2021/valid/'+ path.split('/')[-1] + 'data_f32b0.pkl'
    mask = images.sum(-1) > 0
    for k in range(4):
...
def doit(dset):
    ...
    paths = [os.path.join(sub, name + '_') for sub, name in zip(subjects, names)]
    print(paths)
    for path in paths:
