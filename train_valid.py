...
from models.TransBTS.Conformer_Unet_final import Conformer
#from models.TransBTS.Vnet import Vnet
...
from valid import validate_softmax
...
# DataSet Information
parser.add_argument('--root', default='data/brats2020_new', type=str)
...
def main_worker():
    ...
    if args.local_rank == 0:
        checkpoint_dir = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'checkpoint2', args.experiment+args.date)
    ...
    train_list = os.path.join(args.root,  args.train_file)
    ...
    valid_list = os.path.join(args.root, args.valid_file)
    ...
    for epoch in range(args.start_epoch, args.end_epoch):
        ...
        if (epoch+1) % 20 == 0:
            with torch.no_grad():
                metric, WT, TC, ET = validate_softmax(valid_loader=valid_loader,
                                 model=model,
                                 names=valid_set.names
                                 )
                if metric > best_dice:
                    best_dice = metric
                    final_name = os.path.join(checkpoint_dir, 'model_best.pth')
                    torch.save({
                        ...
                        final_name)
                    print("save best model!", "WT:{:.4f}, TC:{:.4f}, ET:{:.4f}, avg:{:.4f}".format(WT, TC, ET, metric))
