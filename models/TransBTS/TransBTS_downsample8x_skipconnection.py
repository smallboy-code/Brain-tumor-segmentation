...
class TransformerBTS(nn.Module):
    ...
    def encode(self, x):
        ...
        if self.conv_patch_representation:
            ...
            x = self.relu(x)
            x = self.conv_x(x)
            x = x.permute(0, 2, 3, 4, 1).contiguous() # x: [1, 16, 16, 16, 512]
            x = x.view(x.size(0), -1, self.embedding_dim) # x: [1, 4096, 512]
        ...
        x = self.position_encoding(x) # x: [1, 4096, 512]
        x = self.pe_dropout(x)

        # apply transformer
        x, intmd_x = self.transformer(x) # x: [1, 4096, 512], intmd_x['0']: # x: [1, 4096, 512]
        x = self.pre_head_ln(x) # x: [1, 4096, 512]
...
class BTS(TransformerBTS):
    ...
    def decode(self, x1_1, x2_1, x3_1, x, intmd_x, intmd_layers=[1, 2, 3, 4]):
        ...
        for i in intmd_layers:
            ...
            # encoder_outputs["z1"] = intmd_x["1"] encoder_outputs["z2"] = intmd_x["3"]
            # encoder_outputs["z3"] = intmd_x["5"] encoder_outputs["z4"] = intmd_x["7"]
        all_keys.reverse()  # [z4, z3, z2, z1]

        x8 = encoder_outputs[all_keys[0]] # x8: [1, 4096, 512], intmd_x["7"]
        x8 = self._reshape_output(x8) # x8: [1, 512, 16, 16, 16]
...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        model.cuda()
        y = model(x) # torch.Size([1, 4, 128, 128, 128])
        print(y.shape)
