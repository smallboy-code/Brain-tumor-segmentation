...
import torch
from models.TransBTS.conformer_new import ConvBlock
...
class Conformer(nn.Module):
    def __init__(self, in_channels, num_classes):
        ...
        self.conformer0 = ConvBlock(in_channels=16, dw_stride=8)
        self.EnDown1 = EnDown(in_channels=16, out_channels=32)

        self.conformer1 = ConvBlock(in_channels=32, dw_stride=4)
        self.EnDown2 = EnDown(in_channels=32, out_channels=64)

        self.conformer2 = ConvBlock(in_channels=64, dw_stride=2)
        self.EnDown3 = EnDown(in_channels=64, out_channels=128)

        self.conformer3_1 = ConvBlock(in_channels=128, dw_stride=1)
        self.conformer3_2 = ConvBlock(in_channels=128, dw_stride=1)
        self.conformer3_3 = ConvBlock(in_channels=128, dw_stride=1)
        self.conformer3_4 = ConvBlock(in_channels=128, dw_stride=1)
    ...
    def forward(self, x):
        ...
        x = self.conformer0(x)
        output['prev1'] = x['out']
        x = self.EnDown1(x['out'])

        x = self.conformer1(x) # [1, 32, 64, 64, 64]
        output['prev2'] = x['out']
        x = self.EnDown2(x['out'])

        x = self.conformer2(x)  # [1, 64, 32, 32, 32]
        output['prev3'] = x['out']
        x = self.EnDown3(x['out'])

        x = self.conformer3_1(x)  # [1, 128, 16, 16, 16]
        x = self.conformer3_2(x['out'])
        x = self.conformer3_3(x['out'])
        x = self.conformer3_4(x['out'])

        x = self.DeUp1(x['out'], output['prev3'])  # [1, 64, 32, 32, 32]
