...
import torch
from models.TransBTS.conformer_notransformer import ConvBlock
...
class Conformer(nn.Module):
    def __init__(self, in_channels, num_classes):
        ...
        #self.EnBlock = EnBlock(in_channels=16)
        self.conformer0 = ConvBlock(in_channels=16)

        self.EnDown1 = EnDown(in_channels=16, out_channels=32)
        self.conformer1 = ConvBlock(in_channels=32)

        self.EnDown2 = EnDown(in_channels=32, out_channels=64)
        self.conformer2 = ConvBlock(in_channels=64)

        self.EnDown3 = EnDown(in_channels=64, out_channels=128)
        self.conformer3 = ConvBlock(in_channels=128)
        ...
        self.Deconformer1 = ConvBlock(in_channels=64)

        self.DeUp2 = DeUp_Cat(in_channels=64, out_channels=32)
        self.Deconformer2 = ConvBlock(in_channels=32)

        self.DeUp3 = DeUp_Cat(in_channels=32, out_channels=16)
        self.Deconformer3 = ConvBlock(in_channels=16)
    ...
    def forward(self, x):
        ...
        x = self.conformer3(x)  # [1, 128, 16, 16, 16]

        x = self.EnBlock4_1(x['out'])
        x = self.EnBlock4_2(x)
        x = self.EnBlock4_3(x)
        x = self.EnBlock4_4(x)
