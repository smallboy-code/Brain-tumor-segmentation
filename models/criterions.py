...
def softmax_dice2(output, target):
    ...
    return loss1 + loss2 + loss3 + 0.1*loss0, 1-loss1.data, 1-loss2.data, 1-loss3.data
...
def fdl(output, target, eps=1e-5, bat=1.5):
    target = target.float()
    num = 2 * (output * target).sum() + eps
    den = output.sum() + target.sum() + eps
    return 1.0 - (num/den) ** (1/bat)

def FDL(output, target):
    '''
    The dice loss for using softmax activation function
    :param output: (b, num_class, d, h, w)
    :param target: (b, d, h, w)
    :return: softmax dice loss
    '''
    loss0 = fdl(output[:, 0, ...], (target == 0).float())
    loss1 = fdl(output[:, 1, ...], (target == 1).float())
    loss2 = fdl(output[:, 2, ...], (target == 2).float())
    loss3 = fdl(output[:, 3, ...], (target == 4).float())

    return loss1 + loss2 + loss3, 1-loss1.data, 1-loss2.data, 1-loss3.data


def dice_focal(output, target):
    d_loss, loss1, loss2, loss3 = softmax_dice2(output, target)
    f_loss, _, _, _ = Dual_focal_loss(output, target)

    return d_loss + f_loss, loss1, loss2, loss3
